...
hue_by_color_overall <- danish %>% 
  group_by(Color_label_matched, Condition) %>% 
  summarise(max(Rel_dif), min(Rel_dif), mean(Rel_dif), max(Rel_dif_2),min(Rel_dif_2), mean(Rel_dif_2), max(Hue_matched), min(Hue_matched), mean(Hue_matched), mean(Seed), mean(Focal), mean(Secondary_focal))
...
#CONDITION 1
#Rescale Chains
danish$Chain <- rescale(as.numeric(danish$Chain), to = c(1, 5))
danish$Chain <- as.factor(danish$Chain)

cond_1_hue <- ggplot(danish, aes(x = Generation, y = Rel_dif, group = Chain, color = Chain))+
...
cond_1_sat <- ggplot(danish, aes(x = Generation, y = Saturation_matched, group = Chain, color = Chain))+
  geom_line(stat="summary") +
  expand_limits(y=0) +
  xlab("Generation") + 
  ylab("Saturation %") +
...
# for chain 0 in condition 2 the slope has a very steep drop of -24
...
# it looks like chain 2 and 3 are the sinners, as they only have positive slopes, where all the others have negative slopes
# chain two seems to have an especially steep positive slope for condition 2
# chain four seems to have an especially steep negative slope for condition 2
...
Testing the post hoc analysis that for the seed colors that converges in the opporsite direction, they might converge towards their secondary intermediate hues instead of their primary foci
...
# Save the plot to a pdf
ggsave("accuracy_hue_no_outs.pdf")
```
...
# Save the plot to a pdf
ggsave("accuracy_sat_no_outs.pdf")
