...
stats_hue <- danish %>% 
  ...
  summarise(max(Rel_dif),min(Rel_dif),mean(Hue_matched), sd(Rel_dif), mean(Seed), mean(Focal), mean(Accuracy_hue), sd(Accuracy_hue), mean(Confidence)) %>% 
  arrange(Color_label_matched)

stats_hue2 <- data %>% 
  group_by(Color_label_matched, Condition) %>% 
  summarise(max(Rel_dif_2),min(Rel_dif_2), mean(Rel_dif_2), sd(Rel_dif_2), mean(Secondary_focal))


#Saturation
stats_sat <- data %>% 
  group_by(Color_label_matched, Condition) %>% 
  summarise(max(Saturation_matched),min(Saturation_matched),mean(Saturation_matched), sd(Saturation_matched), mean(Accuracy_sat))
...
danish$rel_dif_abs <- abs(danish$Rel_dif)
hue <- lmer(
...
hue2 <- lmer(
  rel_dif_abs ~ Generation + Condition + Generation:Condition +
  (1 + Generation + Condition + Generation:Condition | Color_label_matched) + 
  (1 + Generation + Condition + Generation:Condition| Chain) +
  (1 + Generation + Condition + Generation:Condition| Chain:Subject),
  data = danish, 
